...
function CalculatorTile(props: { href: string; title: string; subtitle: string }) {
  return (
    <Link
      href={props.href}
      className="group relative flex h-full flex-col rounded-2xl border border-gray-200 bg-white p-6 text-center shadow-sm transition hover:shadow-md"
    >
      ...
      <div className="mt-1 flex-1 text-sm text-gray-600">{props.subtitle}</div>
      <div className="mt-3 text-sm text-blue-600 group-hover:underline">Open Calculator →</div>
    </Link>
  );
}
    ...
    <main className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      ...
      <section className="mx-auto flex max-w-4xl flex-col items-center px-6 pb-12 pt-8">
        ...
        <p className="mb-8 max-w-2xl text-center text-gray-600">
          Select a calculator below to continue.
        </p>

        <div className="w-full rounded-3xl bg-gray-100/70 p-5">
          <div className="mx-auto grid max-w-3xl gap-6 sm:grid-cols-2">
            <CalculatorTile
              href="/98g/application_rate"
              title="98G Application Rate"
              subtitle="Calculate recommended 98G application rate for correction or maintenance"
            />
            <CalculatorTile
            ...
            <CalculatorTile
              ...
              title="Soil pH Nutrient Efficiency"
              subtitle="Estimate fertilizer dollars at risk at different soil pH levels"
            />
            <CalculatorTile
        ...
        <div className="mt-8 text-center">
          <Link
            href="/"
            className="inline-flex items-center rounded-lg border border-red-200 bg-red-50 px-5 py-3 text-base font-medium text-red-700 shadow-sm transition hover:bg-red-100"
          >
